...
let ultimaAtualizacaoNotificacoes: Date | null = null;
let intervaloAtualizacao: number | null = null;
...
function inicializarComponenteNotificacoes(): void {
    const iconesNotificacao = document.querySelectorAll<HTMLElement>('.icone-notificacao');
    iconesNotificacao.forEach(icone => {
        icone.addEventListener('click', (e: Event) => {
            e.preventDefault();
            e.stopPropagation();
            ...
            if (menuLateral && menuLateral.classList.contains('ativo')) {
                if (typeof (window as any).fecharMenuLateral === 'function') {
                    (window as any).fecharMenuLateral();
                }
            }
    ...
    if (lista) {
        lista.addEventListener('click', (e: Event) => {
            const alvo = e.target as HTMLElement;
            const notificacao = alvo.closest('.notificacao') as HTMLElement | null;
    ...
    if (btnFechar) {
        btnFechar.addEventListener('click', (e: Event) => {
            e.preventDefault();
    ...
    if (btnMarcarLidas) {
        btnMarcarLidas.addEventListener('click', (e: Event) => {
            e.preventDefault();
...
function togglePainelNotificacoes(): void {
...
function mostrarPainelNotificacoes(): void {
    const painel = document.getElementById('painel-notificacoes');
    if (!painel) {
        console.error('Painel de notificações não encontrado!');
        return;
...
function ocultarPainelNotificacoes(): void {
    const painel = document.getElementById('painel-notificacoes');
    if (!painel) return;
...
function fecharPainelAoClicarFora(e: MouseEvent): void {
    const painel = document.getElementById('painel-notificacoes');
    const icone = document.querySelector('.icone-notificacao');
    const alvo = e.target as Node;

    if (painel && !painel.contains(alvo) && (!icone || !icone.contains(alvo))) {
        ocultarPainelNotificacoes();
    }
}

async function verificarNotificacoes(): Promise<void> {
    const usuarioId = (document.getElementById('usuarioId') as HTMLInputElement | null)?.value;
    ...
    try {
        ...
        const quantidade: number = await response.json();
...
function atualizarContadorNotificacoes(quantidade: number): void {
    contadorNotificacoes = quantidade;

    const contadores = document.querySelectorAll<HTMLElement>('.contador-notificacoes');
    contadores.forEach(contador => {
        if (quantidade > 0) {
            contador.textContent = quantidade.toString();
            contador.style.display = 'flex';

            const sinos = document.querySelectorAll<HTMLElement>('.sino-svg');
            sinos.forEach(sino => {
...
function ocultarContadorNotificacoes(): void {
    const contadores = document.querySelectorAll<HTMLElement>('.contador-notificacoes');
    contadores.forEach(contador => {
        contador.style.display = 'none';
    });
...
async function marcarComoLida(id: string | number): Promise<void> {
    try {
...
async function marcarTodasComoLidas(): Promise<void> {
    try {
        ...
        document.querySelectorAll('.item-notificacao').forEach(item => {
            (item as HTMLElement).classList.remove('notificacao-nao-lida');
        });
...
function exibirMensagem(mensagem: string, tipo?: 'success' | 'error' | 'info'): void {
    if (typeof toastr !== 'undefined') {
        switch (tipo) {
            case 'success':
...
(window as any).inicializarComponenteNotificacoes = inicializarComponenteNotificacoes;
(window as any).togglePainelNotificacoes = togglePainelNotificacoes;
(window as any).mostrarPainelNotificacoes = mostrarPainelNotificacoes;
(window as any).ocultarPainelNotificacoes = ocultarPainelNotificacoes;
(window as any).carregarNotificacoes = carregarNotificacoes;
(window as any).marcarTodasComoLidas = marcarTodasComoLidas;
